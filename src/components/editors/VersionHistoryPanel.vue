<template>
  <div class="absolute inset-0 z-10 flex bg-background">
      ...
      <ScrollArea class="flex-1 min-h-0">
        <div class="p-2 flex flex-col gap-0.5">
          <template v-if="loading">
            <div v-for="n in 4" :key="n" class="h-10 rounded bg-muted animate-pulse" />
          </template>

          <p v-else-if="versions.length === 0" class="px-2 py-1.5 text-xs text-muted-foreground">
            No history yet.
          </p>
          ...
          <button
            ...
            :class="[
              ...
              selectedVersion === v.version ? 'bg-accent text-accent-foreground' : 'hover:bg-muted',
            ]"
    ...
    <div class="flex-1 min-w-0 flex flex-col min-h-0">
      <!-- Preview toolbar -->
      <div class="flex items-center gap-2 px-3 h-10 shrink-0 border-b border-border">
        ...
        <Button size="sm" :disabled="selectedVersion === null || isActing" @click="restore">
...
async function selectVersion(meta: TemplateVersionMeta) {
  ...
  try {
    const snapshot = await templateService.getVersion(props.templateId, meta.version)
...
const statusMessage = ref<{ type: 'info' | 'error'; text: string } | null>(null)
...
async function restore() {
  ...
  isActing.value = true
  try {
    const restored = await templateService.restoreVersion(props.templateId, selectedVersion.value)
    setStatus(
      'info',
