...
async function confirm(): Promise<void> {
  ...
  const node = await createFile(selectedFolderPath.value, name.value.trim(), extension, content)
  emit('created', node)
  ...
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>Create a file in the selected workspace folder.</DialogDescription>
      </DialogHeader>
      ...
      <div class="flex flex-col gap-3 py-2">
        ...
        <div class="space-y-1.5">
          ...
          <select
            ...
            class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus:outline-none focus:ring-1 focus:ring-ring"
          >
            <option v-for="(type, index) in FILE_TYPES" :key="type.label" :value="index">
              {{ type.label }}
            </option>
          </select>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <Input v-model="name" placeholder="invoice" @keydown.enter="confirm" />
