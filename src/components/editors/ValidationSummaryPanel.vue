...
const totalErrors = computed(() => props.files.reduce((sum, f) => sum + f.result.errors.length, 0))
...
const badgeLabel = computed(() => {
  ...
  if (totalErrors.value)
    parts.push(`${totalErrors.value} error${totalErrors.value !== 1 ? 's' : ''}`)
  if (totalWarnings.value)
    parts.push(`${totalWarnings.value} warning${totalWarnings.value !== 1 ? 's' : ''}`)
  ...
  <div class="border-t border-border bg-background text-xs">
    ...
    >
      <div class="flex items-center gap-2">
        ...
        <span v-if="totalErrors > 0" class="flex items-center gap-0.5 text-destructive">
          <AlertCircle class="h-3 w-3" />
          {{ totalErrors }}
        </span>
        <span v-if="totalWarnings > 0" class="flex items-center gap-0.5 text-yellow-500">
    ...
    <div v-if="isOpen" class="max-h-40 overflow-y-auto">
      <template v-for="entry in files" :key="entry.fileId">
        <template v-if="entry.result.errors.length || entry.result.warnings.length">
          <!-- File header -->
          <div
          ...
          <!-- Diagnostics -->
          <button
            v-for="(diag, idx) in [...entry.result.errors, ...entry.result.warnings]"
            :key="idx"
            type="button"
          ...
          >
            ...
            />
            <AlertTriangle v-else class="mt-0.5 h-3 w-3 shrink-0 text-yellow-500" />
            <span class="text-foreground flex-1 break-words">{{ diag.message }}</span>
      ...
      <div
        v-if="files.every((f) => !f.result.errors.length && !f.result.warnings.length)"
