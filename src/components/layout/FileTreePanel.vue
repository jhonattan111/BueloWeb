...
function requestRename(node: FileNode): void {
  const defaultValue =
    node.type === 'file'
      ? node.name.replace(new RegExp(`${node.extension.replace('.', '\\.')}$$`, 'i'), '')
      : node.name
  ...
  <div class="flex flex-col h-full min-h-0">
    <div class="flex items-center justify-between px-3 h-10 shrink-0 border-b border-border">
      ...
      <div class="flex items-center gap-0.5">
        ...
        <Button variant="ghost" size="icon-sm" aria-label="New folder" @click="openNewFolder(null)">
          <FolderPlus class="size-4" />
        </Button>
        <Button variant="ghost" size="icon-sm" aria-label="New file" @click="openNewFile(null)">
          <Plus class="size-4" />
        </Button>
      </div>
    </div>
    ...
    <ScrollArea class="flex-1 min-h-0">
      <div class="p-1">
        <template v-if="isLoading && tree.length === 0">
          <div v-for="n in 5" :key="n" class="h-6 rounded bg-muted animate-pulse mb-1" />
        </template>

        <template v-else-if="tree.length === 0">
          <div class="px-2 py-4 text-center">
            <p class="text-xs text-muted-foreground mb-2">Workspace is empty.</p>
            <div class="flex flex-col items-center gap-2">
              <Button size="sm" :disabled="creatingExamples" @click="emit('loadExamples')">
                <Loader2 v-if="creatingExamples" class="size-3.5 mr-1 animate-spin" />
                <Sparkles v-else class="size-3.5 mr-1" />
                Load examples
              </Button>
              <div class="flex items-center justify-center gap-2">
                <Button size="sm" variant="outline" @click="openNewFolder(null)">New folder</Button>
                <Button size="sm" variant="outline" @click="openNewFile(null)">New file</Button>
              </div>
            </div>
          </div>
        </template>
    ...
    <AlertDialog :open="deleteDialog.open" @update:open="deleteDialog.open = $event">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Delete "{{ deleteDialog.node?.name }}"?</AlertDialogTitle>
          <AlertDialogDescription>This action cannot be undone.</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel @click="deleteDialog.open = false">Cancel</AlertDialogCancel>
    ...
    <Dialog :open="renameDialog.open" @update:open="renameDialog.open = $event">
      <DialogContent>
        ...
        <Input v-model="renameDialog.value" placeholder="New name" @keydown.enter="confirmRename" />
        <DialogFooter>
          <Button variant="outline" @click="renameDialog.open = false">Cancel</Button>
          <Button :disabled="!renameDialog.value.trim()" @click="confirmRename">Rename</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

    <Dialog :open="newFolderDialog.open" @update:open="newFolderDialog.open = $event">
      <DialogContent>
        <DialogHeader>
          <DialogTitle>New Folder</DialogTitle>
          <DialogDescription>Create a folder in the workspace tree.</DialogDescription>
        </DialogHeader>
        <Input
        ...
        />
        <DialogFooter>
          <Button variant="outline" @click="newFolderDialog.open = false">Cancel</Button>
          <Button :disabled="!newFolderDialog.name.trim()" @click="confirmNewFolder">Create</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
