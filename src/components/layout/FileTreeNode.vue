...
<template>
  <div>
    <button
      ...
      :class="selectedId === node.id ? 'bg-accent/40 text-foreground' : 'text-muted-foreground'"
      :style="{ paddingLeft: `${depth * 12 + 4}px` }"
      @click="onClick"
      @contextmenu="onContextMenu"
    >
      <span v-if="node.type === 'folder'" class="shrink-0 text-muted-foreground">
      ...
      <component
        :is="node.type === 'folder' ? (expanded ? FolderOpen : Folder) : fileIcon(node)"
        class="size-3.5 shrink-0"
        :class="node.type === 'folder' ? 'text-muted-foreground' : iconColor(node)"
      />

      <span class="truncate min-w-0 flex-1 font-medium">{{ node.name }}</span>

      <span v-if="badge === 'error'" class="shrink-0 w-1.5 h-1.5 rounded-full bg-destructive" />
      <span
    ...
    <div v-if="node.type === 'folder' && expanded">
      <FileTreeNode
        ...
        @contextmenu="(nodeValue, event) => $emit('contextmenu', nodeValue, event)"
      />
    </div>
  </div>
</template>
