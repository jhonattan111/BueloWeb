<template>
  <div class="flex h-full min-h-0 flex-col">
    <!-- Header -->
    <div class="flex items-center justify-between px-2 h-10 shrink-0 border-b border-border gap-1">
      ...
      <div class="flex items-center gap-0.5">
        ...
        <Button variant="ghost" size="icon" class="size-6" title="New template" @click="onNew">
          <Plus class="size-3.5" />
        </Button>
        <Button
          variant="ghost"
        ...
        >
          <RefreshCw class="size-3.5" :class="{ 'animate-spin': store.isLoading }" />
        </Button>
      </div>
    </div>
    ...
    <div class="px-2 py-1.5 shrink-0 border-b border-border">
      <div class="relative">
        ...
        />
        <Input v-model="search" placeholder="Search templates…" class="h-6 text-xs pl-6 pr-2" />
      </div>
    </div>
    ...
    <ScrollArea class="flex-1 min-h-0">
      ...
      <div v-else-if="!filtered.length" class="px-3 py-6 text-center">
        <p class="text-[11px] text-muted-foreground">
          {{
            search ? 'No templates match your search.' : 'No templates yet. Click + to create one.'
          }}
        </p>
      </div>
      ...
      <div v-else class="py-1">
        <div
          ...
          :class="{
            'bg-accent text-accent-foreground': store.activeTemplateId === tpl.id,
          }"
          @click="onSelect(tpl.id)"
        >
          <FileCode class="size-3.5 shrink-0 text-blue-400" />
          <span v-if="renamingId !== tpl.id" class="flex-1 min-w-0 text-[12px] truncate">{{
            tpl.name
          }}</span>
          <input
            v-else
    ...
    >
      <DialogContent class="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>Enter a name for the new C# report template.</DialogDescription>
        </DialogHeader>
        <div class="space-y-3 py-2">
          ...
          <div class="flex gap-3">
            <label class="flex items-center gap-1.5 text-xs cursor-pointer">
              <input v-model="newFormat" type="radio" value="pdf" class="accent-primary" />
              PDF
            </label>
            <label class="flex items-center gap-1.5 text-xs cursor-pointer">
              <input v-model="newFormat" type="radio" value="excel" class="accent-primary" />
              Excel
            </label>
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" size="sm" @click="newDialogOpen = false">Cancel</Button>
          <Button size="sm" :disabled="!newName.trim() || store.isLoading" @click="executeNew"
            >Create</Button
          >
        </DialogFooter>
      </DialogContent>
...
async function handleImportFile(event: Event): Promise<void> {
  ...
  try {
    ...
    const name =
      (parsed.name ?? file.name.replace(/\.(blt\.json|json|blt)$/i, '')).trim() ||
      'Imported Template'
    await store.createTemplate({
      name,
